import { normalizeFaceDescriptor } from '@/lib/faceDescriptor';
...
export async function captureFace(video) {
  ...
  // Normalize in the browser so descriptor distances are consistent across
  // Android/iOS and WebGL/WASM backends.
  const faceDescriptor = normalizeFaceDescriptor(Array.from(face.descriptor));
  if (!faceDescriptor) throw new Error('Face embedding is invalid. Please try again.');
  return {
