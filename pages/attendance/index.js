...
export default function AttendanceKiosk() {
  ...
  async function verify(faceDescriptor) {
    setResult(null);
  ...
  return <main className="mx-auto min-h-screen max-w-3xl p-4 sm:p-8"><div className="mb-6 text-center"><div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-2xl bg-emerald-700 font-bold text-white">TA</div><h1 className="m-0 text-2xl font-bold">Teacher Attendance</h1><p className="text-slate-500">Today — {new Date().toLocaleDateString('en-IN', { timeZone: IST, day: '2-digit', month: 'long', year: 'numeric' })}</p></div><section className="card p-4 sm:p-6"><h2 className="mt-0 text-lg">Face Verification</h2><FaceCamera onCapture={verify}/>{result && <div className="mt-4 rounded-xl bg-emerald-50 p-4 text-emerald-900"><b>{result.action === 'in' ? 'IN marked successfully' : 'OUT marked successfully'}</b><p className="mb-0 mt-1 text-lg font-bold">{time(result.action === 'in' ? result.attendance.in_time : result.attendance.out_time)}</p></div>}</section><section className="card mt-5 p-5"><h2 className="mt-0 text-lg">Today’s Activity</h2>{activity.length ? activity.map((teacher) => { const record = teacher.attendance; return <div key={teacher.id} className="flex justify-between border-t py-3 text-sm"><b>{teacher.full_name}</b><span>{record.out_time ? 'OUT' : 'IN'} {time(record.out_time || record.in_time)}</span></div>; }) : <p className="text-sm text-slate-500">No attendance activity yet.</p>}</section></main>;
}
